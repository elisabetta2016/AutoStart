...
echo -e "$GRN Moving workspace to $RED master $NC"

echo -e "\nPackage $BLU pcl_analyser $NC"
cd $WS/pcl_analyser;git checkout master

echo -e "Package $BLU rover_actions $NC"
cd $WS/rover_actions;git checkout master

echo -e "Package $BLU pc_maker $NC"
cd $WS/pc_maker;git checkout master

echo -e "Package $BLU rover_simulator $NC"
cd $WS/rover_simulator;git checkout master
